...
namespace LazyShell
{
    public class DialoguePreview
    {
        ...
        // accessors
        private Dialogues.DTE[] tables
        {
            get { return Dialogues.Model.DTE; }
        }
        // Constructor
        ...
        private void AddTriangles(int[] pixels)
        {
            ...
            int[] triangle = triangles[0].GetPixels(palette_t);
            for (int i = 0; i < 3; i++)
            {
                location.Y = i * 16 + 4;
                if (drawOptions[i])
            ...
            if (drawPageBreak)
            {
                triangle = triangles[7].GetPixels(palette_t);
